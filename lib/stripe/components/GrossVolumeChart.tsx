...
export function GrossVolumeChart({
  ...
  const generatePath = useMemo(() => {
    ...
    const stepX = sortedData.length <= 1 ? 0 : chartWidth / (sortedData.length - 1);
    ...
    <Card>
      ...
      <CardContent>
        {isLoading ? (
          ...
          <>
            <div className="mb-4">
              <h3 className="text-2xl font-bold" style={{ color: totalVolume < 0 ? '#ef4444' : '#22c55e' }}>
              ...
              <div className="h-[260px] w-full overflow-x-auto">
                ...
                >
                  ...
                  {lowestValue < 0 && (
                    <line
                      x1={padding.left}
                      y1={padding.top + chartHeight * (1 - Math.abs(0 - lowestValue) / Math.abs(highestValue - lowestValue))}
                      x2={padding.left + chartWidth}
                      y2={padding.top + chartHeight * (1 - Math.abs(0 - lowestValue) / Math.abs(highestValue - lowestValue))}
                      stroke="#888"
                      strokeWidth="1"
                      strokeDasharray="3,3"
                    />
                  ...
                  {sortedData.map((point, i) => {
                    ...
                    const valueRange = Math.abs(highestValue - lowestValue) || 1;
                    const baseline = lowestValue >= 0 ? 0 : lowestValue;
                    const y = padding.top + (chartHeight - ((point.amount - baseline) / valueRange) * chartHeight);
                    ...
                    return (
                      ...
                      >
                        <title>{point.date}: {formatCurrency(point.amount, currency)}</title>
                      </circle>
                    );
                  })}
                    ...
                    if (i === 0 || i === sortedData.length - 1 || i % 4 === 0) {
                      ...
                      return (
                        <text
                          ...
                          x={x}
                          y={svgHeight - 10}
                          textAnchor={i === 0 ? "start" : i === sortedData.length - 1 ? "end" : "middle"}
                          fontSize="10"
                          fill="#888"
                        >
